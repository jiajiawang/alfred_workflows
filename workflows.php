...
* Modified by:  Jiajia Wang (https://github.com/jiajiawang)
* Revised:      2015-06-11
* Version:      0.3
*/
class Workflows {
    ...
    public function toxml( $a=null, $format='array' ) {
        ...
        foreach( $a as $b ):                                // Lop through each object in the array
            ...
            foreach( $c_keys as $key ):                     // For each of those keys
                ...
                elseif ( preg_match('/^(shift|fn|ctrl|alt|cmd)$/', $key) ):
                    $child = $c->addChild('subtitle', $b[$key]);
                    $child->addAttribute('mod', $key);
                elseif ( preg_match('/^(copy|largetype)$/', $key) ):
                    $child = $c->addChild('text', $b[$key]);
                    $child->addAttribute('type', $key);
                else:
                    $c->$key = $b[$key];
                endif;
            endforeach;
    ...
    */
    ...
    /**
    * Description:
    * Helper function that pushs array to results
    *
    * @param $arr - a result array
    * @return array - list of result items
    */
    public function results_push( $arr )
    {
